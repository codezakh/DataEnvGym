...
def execute_cb_code(
    method, inputs_list, outputs_list, timeout, early_stop=False, debug=False
):
    ...
    for index, inputs in enumerate(inputs_list):
        ...
        if debug:
            print(
                f"outputs = {exec_outputs}, test outputs = {outputs}, inputs = {inputs}, {type(inputs)}, {exec_outputs == [outputs]}"
            )
            debug_infos[index] = {
                "inputs": inputs,
                "gt_outputs": outputs,
...
def execute_std_code(
    synthesized_code, inputs_list, outputs_list, timeout, early_stop=False, debug=False
):
    ...
    for i, inputs in enumerate(inputs_list):
                ...
                try:
                    ...
                    try:
                        ...
                        with open(inputs_tmp_file, "w") as ftemp:
                            ftemp.write(inputs)
                        result = subprocess.run(
                            ["python", temp_program_path], text=True, timeout=timeout
                        )
                        assert result.returncode == 0
                        if compare_std_results(
                            open("output.txt").read(), outputs, debug
                        ):
                            exec_code = 1
                        else:
                            exec_code = 0
        ...
        exec_results[i] = (
            ...
            (
                ...
                if exec_code > -3
                else EXECUTION_RESULTS[exec_code].format(code=result.returncode)
            ),
        )
        if exec_code >= 0:
            if debug:
